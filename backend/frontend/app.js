// ============================================================
// Advanced-RAG-learning-system — Frontend Logic
// ============================================================

// ── State ────────────────────────────────────────────────────
const STATE = {
  userId:      localStorage.getItem('advanced_rag_uid')  || null,
  username:    localStorage.getItem('advanced_rag_name') || null,
  email:       localStorage.getItem('advanced_rag_email')|| null,
  docId:       localStorage.getItem('advanced_rag_docid')|| null,
  docFilename: localStorage.getItem('advanced_rag_docfn')|| null,
  ...
  /** Sarvam chat: "105b" | "30b" — persisted under `advanced_rag` */
  llmVariant:  localStorage.getItem('advanced_rag') === '105b' ? '105b' : '30b',
  refreshBundle: {
...
function setLlmVariant(v) {
  ...
  localStorage.setItem('advanced_rag', STATE.llmVariant);
  syncLlmModelToggle();
}
...
function _saveDocState() {
  if (STATE.docId) {
    localStorage.setItem('advanced_rag_docid', STATE.docId);
    localStorage.setItem('advanced_rag_docfn', STATE.docFilename || '');
  } else {
    localStorage.removeItem('advanced_rag_docid');
    localStorage.removeItem('advanced_rag_docfn');
  }
}
...
function doLogout() {
  ['advanced_rag_uid','advanced_rag_name','advanced_rag_email','advanced_rag_docid','advanced_rag_docfn'].forEach(k => localStorage.removeItem(k));
...
function persist() {
  localStorage.setItem('advanced_rag_uid',   STATE.userId||'');
  localStorage.setItem('advanced_rag_name',  STATE.username||'');
  localStorage.setItem('advanced_rag_email', STATE.email||'');
}
